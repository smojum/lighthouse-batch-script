...
function sitesInfo(options) {
    ...
    if (options.file) {
        try {
            ...
            sites = contents.trim().split('\n').map(value => options.baseuri + value);
            console.log(sites)
        } catch (e) {
    ...
    return sites.map(url => {
        ...
        return info;
        if (options.html) info.csv = `${name}${CSV_EXT}`;
        return info
    })
}
...
function updateSummary(filePath, summary, outcome, options) {
    ...
    return {
        ...summary,
        ...getPerformanceMetrics(report)
        // ...getAverageScore(report)
    }
}

function getPerformanceMetrics(report) {
    let metricsItem = report.audits.metrics.details.items[0];
    return metricsItem
}

function getPerformanceAutdits() {
